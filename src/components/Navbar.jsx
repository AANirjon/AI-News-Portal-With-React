...
const Navbar = ({ setArticles }) => {
    ...
    return (
        <div className='fixed w-full bg-white dark:bg-slate-900 z-10 shadow-md'>
            <div className='max-w-7xl mx-auto px-4 py-3 flex items-center justify-between'>
                ...
                <div className='hidden md:flex space-x-6'>
                    {
                        links.map((link) => {
                            return <Link to={`/${link.toLowerCase()}`} key={link} className='relative text-lg font-medium text-gray-800 dark:text-gray-200 transition-transform duration-300 hover:scale-105 after:absolute after:left-1/2 after:-bottom-1 after:h-[2px] after:w-0   after:bg-blue-500 dark:after:bg-blue-400 after:transition-all after:duration-300 after:ease-in-out hover:after:left-0 hover:after:w-full'>
                                {link}
                            </Link>
                        })
                    }
